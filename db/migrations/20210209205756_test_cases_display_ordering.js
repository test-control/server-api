...
const dataSpQueries = {
  mssql: [
    ...
    begin
        ...
        select @last_tree_id = test_suite_id from inserted;
        select @last_id = id from inserted;
        select @last_order = display_order from ${tableName} where test_suite_id = @last_tree_id order by display_order desc offset 0 rows fetch next 1 rows only;
    ...
    begin
        ...
        select @fromDisplayOrder = display_order, @fromTreeId = test_suite_id from ${tableName} where id = @fromOrder;
        select @targetDisplayOrder = display_order, @targetTreeId = test_suite_id from ${tableName} where id = @targetOrder;
       
        if @targetDisplayOrder < @fromDisplayOrder 
        begin
            update ${tableName} set display_order = display_order + 1 where test_suite_id = @fromTreeId and display_order >= @targetDisplayOrder and display_order < @fromDisplayOrder;
            update ${tableName} set display_order = @targetDisplayOrder where id = @fromOrder;
        end
        else
        begin
            update ${tableName} set display_order = display_order - 1 where test_suite_id = @fromTreeId and display_order > @fromDisplayOrder and display_order <= @targetDisplayOrder;
            update ${tableName} set display_order = @targetDisplayOrder where id = @fromOrder;
        end;
    end;
    `
  ...
  mysql: [
    ...
    begin
        ...
        select display_order into last_order from ${tableName} where test_suite_id = NEW.test_suite_id order by display_order desc limit 1;
    ...
    begin
        ...
        select display_order, test_suite_id into fromDisplayOrder, fromTreeId from ${tableName} where id = fromOrder;
        select display_order, test_suite_id into targetDisplayOrder, targetTreeId from ${tableName} where id = targetOrder;
...
begin
    select display_order, test_suite_id into fromRow from ${tableName} where id = fromOrder;
    ...
    if targetRow.display_order < fromRow.display_order then
        update ${tableName} set display_order = display_order + 1 where test_suite_id = fromRow.test_suite_id and display_order >= targetRow.display_order and display_order < fromRow.display_order;
        update ${tableName} set display_order = targetRow.display_order where id = fromOrder;
    else
        update ${tableName} set display_order = display_order - 1 where test_suite_id = fromRow.test_suite_id and display_order > fromRow.display_order and display_order <= targetRow.display_order;
        update ${tableName} set display_order = targetRow.display_order where id = fromOrder;
    end if;
